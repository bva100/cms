...
class ApiException extends HttpException {
    ...
    public function __construct($code, $format)
    {
        ...
        $data = array();
        $data['code'] = $code;
        $data['more_info'] = $baseUrl.'/docs/exception/'.$code;
    ...
    public function createMessage($data, $format)
    {
        switch ($format) {
            case 'text/html':
                return '<h1>'.$data['code'].' '.$data['error_type'].'</h1><p>'.$data['message'].'</p><p><a href="'.$data['more_info'].'" target="_blank"/>'.$data['more_info'].'</a></p>';
                break;
